                        ...
                        @foreach ($faqs as $_key => $faq)
                            <div class="accordion__item">
                                <div class="accordion__header" id="heading_{{ $_key }}">
                                    <button class="accordion-button {{ $_key == '0' ? '' : 'collapsed' }}" type="button"
                                        data-bs-toggle="collapse" data-bs-target="#collapse_{{ $_key }}"
                                        aria-expanded="true" aria-controls="collapse_{{ $_key }}">
                                        {{ $faq->lang_question }}
                                    </button>
                                </div>
                                <div id="collapse_{{ $_key }}"
                                    class="accordion-collapse collapse {{ $_key == '0' ? 'show' : '' }}"
                                    aria-labelledby="heading_{{ $_key }}" data-bs-parent="#faqAccordion">
