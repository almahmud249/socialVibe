...
use App\Http\Controllers\Admin\AuthContentController;
...
use App\Http\Controllers\Admin\WebsiteCustomerController;
use App\Http\Controllers\Admin\WebsiteFaqController;
use App\Http\Controllers\Admin\WebsiteFeatureController;
use App\Http\Controllers\Admin\WebsiteGrowthController;
use App\Http\Controllers\Admin\WebsiteIntegrateController;
...
Route::group(['prefix' => localeRoutePrefix()], function () {
    Route::group(['prefix' => 'admin', 'middleware' => ['auth', 'verified', 'adminCheck', 'PermissionCheck']], function () {
        ...
        // social platform
        Route::get('social-platform', [SystemSettingController::class, 'socialPlatform'])->name('social.platform.setting');
        Route::post('social-platform', [SystemSettingController::class, 'saveSPSetting'])->name('social.platform.setting');
        ...
        Route::resource('countries', CountryController::class)->except(['create.blade.php', 'show', 'update']);
        Route::post('countries/update/{id}', [CountryController::class, 'update'])->name('countries.update');
        Route::resource('cities', CityController::class)->except(['create.blade.php', 'show', 'update']);
        ...
        // our Customer
        Route::resource('website/customers', WebsiteCustomerController::class)->except(['show']);

        // integrate section
        Route::resource('website/integrate', WebsiteIntegrateController::class)->except(['show']);
        Route::post('website/integrate/store', [WebsiteIntegrateController::class, 'integrateContent'])->name('integrate.content.store');

        // website advantage
        Route::resource('website/advantage', WebsiteAdvantageController::class);
        // auth content
        Route::resource('auth/content', AuthContentController::class);
        // website faq
        Route::resource('website/faqs', WebsiteFaqController::class)->except(['show']);

        Route::resource('website/growths', WebsiteGrowthController::class)->except(['show']);
        // website cta
    ...
    Route::group(['prefix' => 'admin', 'middleware' => ['auth', 'verified']], function () {
        ...
        Route::post('website-growth-status', [WebsiteGrowthController::class, 'statusChange'])->name('website.growth-status.change');
        Route::post('website-integrate-status', [WebsiteIntegrateController::class, 'statusChange'])->name('website.website-integrate-status');
        Route::post('website-advantage-status', [WebsiteAdvantageController::class, 'statusChange'])->name('website-advantage-status');
        Route::post('auth-content-status', [AuthContentController::class, 'statusChange'])->name('auth-content-status');
        ...
        Route::post('ourClient-status', [WebsiteCustomerController::class, 'statusChange'])->name('ourClient.status.change');
        Route::post('accounts-status', [AccountsController::class, 'statusChange'])->name('accounts.status');
    });
    // for ajax request
