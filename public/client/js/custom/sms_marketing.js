(function ($) {
	...
	$(document).on("ready", function () {
			...
			Swal.fire({
				...
				preConfirm: () => {
					...
					return axios
						...
						.catch((error) => {
							console.error(error);
							Swal.fire("Error", "An error occurred while processing the request.");
						});
				},
			});
		};
		...
		$(document).on("click", ".__js_update_status", function () {
			confirmationAlert($(this).data("url"), $(this).data("status"), "Yes, Update It!");
		});
		...
		if ($messageBody.length > 0) {
			...
			const updateTotalSMS = () => {
				const messageLength = $messageBody.val().length;
				const smsCount = messageLength <= 160 ? totalContacts : Math.ceil(messageLength / 160) * totalContacts;
		...
		$("#campaign_store").on("submit", function (event) {
			if ($("#send_scheduled").prop("checked") && !$("#schedule_time").val().trim()) {
		...
		$("#contact_id").select2(getLiveSearch($("#contact_id").data("url"), "Select contact"));
			...
			$.ajax({
				...
				error: (xhr) => {
					...
					$.each(xhr.responseJSON.errors || [], (key, value) => {
						$(`#${key}`).addClass("is-invalid").siblings(".invalid-feedback").html(value[0]).show();
					});
				},
			});
		});
